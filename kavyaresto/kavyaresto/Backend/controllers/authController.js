...
async function trySendWithFallback(mailOptions) {
  ...
  return mockTransport.sendMail(mailOptions);
}
...
exports.resendOtp = async (req, res) => {
  try {
    ...
    if (!result.ok) {
      // In dev, return the underlying send error for debugging
      const err = result.error
      console.error('Resend OTP send error:', err)
      const errMessage = err && err.message ? err.message : String(err)
      // include sendgrid response body if available
